<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\PurchaseOrder;
use App\Models\Company;
use App\Models\SalesOrder;
use App\Models\PoItem;
use App\Models\SoItem;
use App\Models\Category;
use App\Models\ConvRate;
use App\Models\SubCategory;
use App\Models\Dispatch;
use App\Models\FreightRate;
use Illuminate\Support\Facades\DB;

class NewDispatchController extends Controller
{

    function __construct()
    {
        $this->middleware('permission:Dispatch-index', ['only' => ['index']]);
        $this->middleware('permission:Dispatch-create', ['only' => ['create', 'storeDispatch']]);
        $this->middleware('permission:Dispatch-edit', ['only' => ['updateDispatch', 'editDispatch']]);
        $this->middleware('permission:Dispatch-delete', ['only' => ['destroyDispatch']]);
    }

    public function index()
    {
        $disaptch_data = Dispatch::leftjoin('so_items', 'dispatches.so_item_id', '=', 'so_items.id')
            ->leftjoin('po_items', 'dispatches.po_item_id', '=', 'po_items.id')
            ->leftjoin('companies as po_company', 'dispatches.po_company_id', '=', 'po_company.id')
            ->leftjoin('companies as so_company', 'dispatches.so_company_id', '=', 'so_company.id')
            ->leftjoin('sales_orders', 'dispatches.so_id', '=', 'sales_orders.id')
            ->leftjoin('purchase_orders', 'dispatches.po_id', '=', 'purchase_orders.id')
            ->leftjoin('categories', 'dispatches.category_id', '=', 'categories.id')
            ->leftjoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
            ->select(
                'dispatches.*',
                'so_items.so_dispatch_rest_qty',
                'po_items.po_dispatch_rest_qty',
                'po_company.company_name as po_company',
                'so_company.company_name as so_company',
                'sales_orders.so_number',
                'purchase_orders.id as po_id',
                'sales_orders.id as so_id',
                'purchase_orders.document_number as po_number',
                'sales_orders.date as so_date',
                'purchase_orders.date as po_date',
                'categories.name as category_name',
                'subcategories.sub_category as sub_category_name',
                'po_items.po_item_no',
                'so_items.so_item_no',
                'po_items.qty as po_qty',
                'so_items.qty as so_qty',
                'dispatches.id as dispatch_id',
                'dispatches.date as dispatch_date',

            )
            ->orderBy('dispatches.date', 'desc')
            ->orderBy('dispatches.id', 'desc')
            ->get();
        return view('newdispatch.index', compact('disaptch_data'));
    }

    public function create()
    {
        $purchase_orders = PurchaseOrder::join('companies', 'companies.id', '=', 'purchase_orders.supplier_id')
            ->get();
        $sales_orders = SalesOrder::all();

        $companies_po = Company::where('type', '!=', 'buyer')->get();
        $companies_so = Company::where('type', '!=', 'supplier')->get();


        return view('newdispatch.create', compact('purchase_orders', 'sales_orders', 'companies_po', 'companies_so'));
    }

    public function create_so()
    {
        $purchase_orders = PurchaseOrder::join('companies', 'companies.id', '=', 'purchase_orders.supplier_id')
            ->get();
        $sales_orders = SalesOrder::all();

        $companies_po = Company::where('type', '!=', 'buyer')->get();
        $companies_so = Company::where('type', '!=', 'supplier')->get();


        return view('newdispatch_so.create', compact('purchase_orders', 'sales_orders', 'companies_po', 'companies_so'));
    }

    public function getPurchaseOrders(Request $request)
    {
        $purchaseOrders = PurchaseOrder::join('companies', 'companies.id', '=', 'purchase_orders.supplier_id')
            ->join('po_items', function ($join) {
                $join->on('po_items.po_id', '=', 'purchase_orders.id')
                    ->whereNotIn('po_items.po_dispatch_item_status', ['Close', 'Pre Closed', 'Cancelled']);
            })
            ->join('categories', 'categories.id', '=', 'po_items.item_category')
            ->where('purchase_orders.supplier_id', $request->company_id)
            ->orderBy('purchase_orders.date', 'asc')
            ->get();

        return response()->json(['purchase_orders' => $purchaseOrders]);
    }

    public function getPoItems(Request $request)
    {
        $poItems = PoItem::join('categories', 'categories.id', '=', 'po_items.item_category')->where('po_id', $request->po_id)->get();
        // dd($poItems);
        return response()->json(['po_items' => $poItems]);
    }

    public function getSalesOrders(Request $request)
    {
        $companyId = $request->company_id;
        $salesOrders = SalesOrder::join('companies', 'companies.id', '=', 'sales_orders.company_id')
            ->join('so_items', function ($join) {
                $join->on('so_items.so_id', '=', 'sales_orders.id')
                    ->whereNotIn('so_items.so_dispatch_item_status', ['Close', 'Pre Closed', 'Cancelled']);
            })
            ->join('categories', 'categories.id', '=', 'so_items.item_category')
            ->where('sales_orders.company_id', $companyId)
            ->orderBy('sales_orders.date', 'asc')
            ->get();
        return response()->json(['salesOrders' => $salesOrders]);
    }

    // Method to get SO Items based on Sales Order ID
    public function getSoItems(Request $request)
    {
        $salesOrderId = $request->sales_order_id;

        // Retrieve items associated with the selected sales order
        $soItems = SoItem::join('categories', 'categories.id', '=', 'so_items.item_category')->where('so_id', $salesOrderId)->get();
        // dd($soItems);

        return response()->json(['soItems' => $soItems]);
    }

    public function getItemDetails(Request $request)
    {
        // dd($request);
        $itemId = $request->item_id;
        $po_items = PoItem::join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
            ->join('categories', 'categories.id', '=', 'po_items.item_category')
            ->select('purchase_orders.*', 'categories.*', 'po_items.*', 'po_items.price as po_price', 'po_items.po_dispatch_rest_qty', 'po_items.po_id')
            ->where('po_item_no', $request->poItemNo)->first();

        // Retrieve the item and its sub-items based on the item ID
        $item = Category::where('id', $itemId)->first();
        $subItems = SubCategory::where('category_id', $itemId)->get();
        $freight_insurance = FreightRate::latest()->first();
        // dd($subItems);

        // Return response with both item and sub-item details
        return response()->json(['item_details' => $item, 'subItems' => $subItems, 'po_items' => $po_items, 'freight' =>  $freight_insurance->freight_rate, 'insurance' =>  $freight_insurance->insurance_rate, 'po_dispatch_rest_qty' =>  $po_items->po_dispatch_rest_qty,]);
    }

    public function getItemDetailsSO(Request $request)
    {
        // dd($request);
        $so_items = SoItem::join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
            ->join('categories', 'categories.id', '=', 'so_items.item_category')
            ->select('sales_orders.*', 'categories.*', 'so_items.*', 'so_items.price as so_price', 'so_items.so_id as so_id', 'categories.id as category_id')
            ->where('so_item_no', $request->so_item_no)->first();

        $sub_items = SubCategory::where('category_id', $so_items->category_id)->get();

        $freight_insurance = FreightRate::latest()->first();


        return response()->json(['so_items' => $so_items, 'freight_insurance' => $freight_insurance, 'subitem' => $sub_items]);
    }

    public function storeDispatch(Request $request)
    {


        // $month = date('m');
        // $year = date('Y');

        // if ($month >= 4) {
        //     $financial_year = $year;
        // } else {
        //     $financial_year = $year - 1;
        // }
        // $last_sail_number = Dispatch::whereYear('created_at', '=', $financial_year)
        //     ->latest('id')
        //     ->first();

        // if ($last_sail_number) {
        //     $max_serial_number = $last_sail_number->dispatch_number;
        //     $last_serial_number = substr($max_serial_number, -4); // Get the last 4 digits
        //     $next_serial_number = str_pad((int) $last_serial_number + 1, 4, '0', STR_PAD_LEFT);
        // } else {
        //     $next_serial_number = '0001';
        // }

        // $doc_number = 'DIS' . $financial_year . $next_serial_number;

        $year = date('Y');
        $month = date('m');

        // Financial year calculation
        if ($month >= 4) {
            $financial_year_start = $year;
            $financial_year_end = $year + 1;
        } else {
            $financial_year_start = $year - 1;
            $financial_year_end = $year;
        }

        // Financial year format
        $financial_year = $financial_year_start;

        // Fetch the latest dispatch number for the current financial year
        $last_dispatch = Dispatch::whereBetween('created_at', [
            "$financial_year_start-04-01 00:00:00",
            "$financial_year_end-03-31 23:59:59",
        ])->orderBy('dispatch_number', 'desc')->first();

        if ($last_dispatch) {
            $max_serial_number = $last_dispatch->dispatch_number;
            $last_serial_number = substr($max_serial_number, -4); // Get the last 4 digits
            $next_serial_number = str_pad((int)$last_serial_number + 1, 4, '0', STR_PAD_LEFT);
        } else {
            $next_serial_number = '0001'; // Default serial number when no records exist
        }

        // Generate Dispatch number with financial year
        $doc_number = 'DIS' . $financial_year . $next_serial_number;

        $rowIdentifiers = [];
        $soDispatchQuantities = [];
        $poDispatchQuantities = [];

        // dd( $request);

        foreach ($request->quantity as $index => $quantity) {
            $rowKey = $request->po_item_number[$index] . '-' .
                $request->sub_cat_id[$index] . '-' .
                $request->so_item_no[$index];

            if (in_array($rowKey, $rowIdentifiers)) {
                return response()->json(['message' => 'Duplicate rows detected for PO Item, Subcategory, Insurance Status, or SO Item.'], 400);
                // return redirect()->back()->with('msg', 'Duplicate rows detected for PO Item, Subcategory, Insurance Status, or SO Item.');
            }

            $rowIdentifiers[] = $rowKey;

            $so_item = SoItem::where('so_item_no', $request->so_item_no[$index])->first();
            $po_item = PoItem::where('po_item_no', $request->po_item_number[$index])->first();

            if (!$so_item || !$po_item) {
                return response()->json(['message' => 'Please select at least one PO Item or SO Item.'], 400);
            }

            // Initialize cumulative dispatched quantities
            $soDispatchQuantities[$request->so_item_no[$index]] = $soDispatchQuantities[$request->so_item_no[$index]] ?? 0;
            $poDispatchQuantities[$request->po_item_number[$index]] = $poDispatchQuantities[$request->po_item_number[$index]] ?? 0;

            // Update cumulative dispatched quantities
            $soDispatchQuantities[$request->so_item_no[$index]] += $quantity;
            $poDispatchQuantities[$request->po_item_number[$index]] += $quantity;


            $dispatchedSOQty = round(floatval($soDispatchQuantities[$request->so_item_no[$index]]), 3);
            $soRestQty = round(floatval($so_item->so_dispatch_rest_qty), 3);

            $dispatchedPOQty = round(floatval($poDispatchQuantities[$request->po_item_number[$index]]), 3);
            $poRestQty = round(floatval($po_item->po_dispatch_rest_qty), 3);

            $dispatchedQty = round(floatval($quantity), 3);

            if ($dispatchedSOQty > $soRestQty) {
                return response()->json([
                    'message' => 'Dispatched quantity for SO Item ' . $so_item->so_item_no . ' exceeds its remaining quantity (' . number_format($soRestQty, 3) . ').'
                ], 400);
            }

            // Validate PO dispatch quantity
            if ($dispatchedPOQty > $poRestQty) {
                return response()->json([
                    'message' => 'Dispatched quantity for PO Item ' . $po_item->po_item_no . ' exceeds its remaining quantity (' . number_format($poRestQty, 3) . ').'
                ], 400);
            }

            // Validate combined dispatch quantity
            if ($dispatchedQty > $soRestQty || $dispatchedQty > $poRestQty) {
                return response()->json([
                    'message' => 'Dispatch Rest Quantity Less than Dispatched Quantity.'
                ], 400);
            }
        }




        //   .............................................................................................................................................

        // Save dispatch data as in your original function
        foreach ($request->quantity as $index => $quantity) {
            $so_item = SoItem::where('so_item_no', $request->so_item_no[$index])
                ->join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->select('sales_orders.*', 'so_items.*', 'so_items.id as so_item_id')
                ->first();
            $po_item = PoItem::where('po_item_no', $request->po_item_number[$index])
                ->join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                ->select('purchase_orders.*', 'po_items.*', 'po_items.id as po_item_id')
                ->first();
            $dispatch = new Dispatch();
            $dispatch->date = $request->date;
            $dispatch->dispatch_number = $doc_number;
            $dispatch->po_company_id =  $po_item->supplier_id;
            $dispatch->so_company_id =  $so_item->company_id;
            $dispatch->po_id = $po_item->po_id;
            $dispatch->so_id = $so_item->so_id;
            $dispatch->po_item_id = $po_item->po_item_id;
            $dispatch->so_item_id = $so_item->so_item_id;
            $dispatch->category_id = $request->cat_id[$index];
            $dispatch->subcategory_id = $request->sub_cat_id[$index];
            $dispatch->dispatched_quantity = $request->quantity[$index];
            $dispatch->conv_rate = $request->conv_rate[$index];
            $dispatch->dispatch_unit_price = $request->dispatch_unit_price_actual[$index];

            $dispatch->dispatch_other = $request->dispatch_fregiht_insuance[$index];
            $dispatch->dispatch_so_other = $request->dispatch_fregiht_insuance[$index];

            $dispatch->dispatch_total = $request->dispatch_total[$index];
            $dispatch->vehicle_number = $request->vehicle_number;

            $dispatch->dispatch_so_unit_price = $so_item->unit_price;

            $dispatch->dispatch_so_total = $request->dispatch_so_total[$index];
            $dispatch->receiver_person = $request->receiver_person;

            $dispatch->remarks = $request->remarks;
            $dispatch->save();


            $actual_so_dispatch_qty = number_format($so_item->so_dispatch_rest_qty - $dispatch->dispatched_quantity, 3);
            $actual_po_dispatch_qty = number_format($po_item->po_dispatch_rest_qty - $dispatch->dispatched_quantity, 3);


            $so_item->update(['so_dispatch_rest_qty' => $actual_so_dispatch_qty]);
            $po_item->update(['po_dispatch_rest_qty' => $actual_po_dispatch_qty]);

            if ($actual_so_dispatch_qty == 0) {
                $so_item->update(['so_dispatch_item_status' => 'Close']);
            } else {
                $so_item->update(['so_dispatch_item_status' => 'Partial Pending']);
            }
            if ($actual_po_dispatch_qty == 0) {
                $po_item->update(['po_dispatch_item_status' => 'Close']);
            } else {
                $po_item->update(['po_dispatch_item_status' => 'Partial Pending']);
            }
        }

        return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);

        // return redirect()->route('dispatch.index')->with('success', 'Dispatch details saved successfully.');
    }


    // public function editDispatch($id)
    // {
    //     $disaptch_data = Dispatch::leftjoin('so_items', 'dispatches.so_item_id', '=', 'so_items.id')
    //         ->leftjoin('po_items', 'dispatches.po_item_id', '=', 'po_items.id')
    //         ->leftjoin('companies as po_company', 'dispatches.po_company_id', '=', 'po_company.id')
    //         ->leftjoin('companies as so_company', 'dispatches.so_company_id', '=', 'so_company.id')
    //         ->leftjoin('sales_orders', 'dispatches.so_id', '=', 'sales_orders.id')
    //         ->leftjoin('purchase_orders', 'dispatches.po_id', '=', 'purchase_orders.id')
    //         ->leftjoin('categories', 'dispatches.category_id', '=', 'categories.id')
    //         ->leftjoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
    //         ->select(
    //             'dispatches.*',
    //             'so_items.so_dispatch_rest_qty',
    //             'po_items.po_dispatch_rest_qty',
    //             'po_company.company_name as po_company',
    //             'so_company.company_name as so_company',
    //             'sales_orders.so_number',
    //             'purchase_orders.document_number as po_number',
    //             'sales_orders.date as so_date',
    //             'purchase_orders.date as po_date',
    //             'categories.name as category_name',
    //             'subcategories.sub_category as sub_category_name',
    //             'po_items.po_item_no',
    //             'so_items.so_item_no',
    //             'po_items.qty as po_qty',
    //             'so_items.qty as so_qty',
    //             'po_items.id as po_item_id',
    //             'so_items.id as so_item_id',
    //             'po_items.unit_price as po_item_unit_price',
    //             'so_items.unit_price as so_item_unit_price',
    //             'dispatches.id as dispatch_id',
    //         )
    //         ->where('dispatches.id', $id)
    //         ->first();

    //     $freight_insurance = FreightRate::latest()->first();

    //     $dispatch_po_price = ($disaptch_data->dispatch_unit_price);
    //     $dispatch_so_price = ($disaptch_data->dispatch_so_unit_price);


    //     $dispatch_po_price_gross = ($disaptch_data->dispatch_unit_price + $disaptch_data->dispatch_other + $disaptch_data->conv_rate);
    //     $dispatch_so_price_gross = ($disaptch_data->dispatch_so_unit_price + $disaptch_data->dispatch_other + $disaptch_data->conv_rate);

    //     // dd($disaptch_data);
    //     $sub_items = SubCategory::where('category_id',  $disaptch_data->category_id)->get();


    //     $so_item = SoItem::join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
    //         ->leftjoin('categories', 'so_items.item_category', '=', 'categories.id')
    //         ->select('sales_orders.*', 'categories.*', 'so_items.*', 'so_items.price as so_price')
    //         ->where('so_items.id', $disaptch_data->so_item_id)->first();


    //     $po_item = PoItem::join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
    //         ->leftjoin('categories', 'po_items.item_category', '=', 'categories.id')
    //         ->select('purchase_orders.*', 'categories.*', 'po_items.*', 'po_items.price as po_price')
    //         ->where('po_items.id', $disaptch_data->po_item_id)->first();

    //     // dd( $so_item,  $po_item);

    //     return view('newdispatch.edit', compact('disaptch_data', 'sub_items', 'so_item', 'po_item', 'dispatch_po_price_gross', 'dispatch_so_price_gross', 'dispatch_po_price', 'dispatch_so_price', 'freight_insurance'));
    // }

    public function editDispatch($dispatch_number)
    {
        // Fetch dispatch data with related joins


        $dispatch_data = Dispatch::leftJoin('so_items', 'dispatches.so_item_id', '=', 'so_items.id')
            ->leftJoin('po_items', 'dispatches.po_item_id', '=', 'po_items.id')
            ->leftJoin('companies as po_company', 'dispatches.po_company_id', '=', 'po_company.id')
            ->leftJoin('companies as so_company', 'dispatches.so_company_id', '=', 'so_company.id')
            ->leftJoin('sales_orders', 'dispatches.so_id', '=', 'sales_orders.id')
            ->leftJoin('purchase_orders', 'dispatches.po_id', '=', 'purchase_orders.id')
            ->leftJoin('categories', 'dispatches.category_id', '=', 'categories.id')
            ->leftJoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
            ->select(
                'dispatches.*',
                'so_items.so_dispatch_rest_qty',
                'po_items.po_dispatch_rest_qty',
                'po_company.company_name as po_company',
                'so_company.company_name as so_company',
                'sales_orders.so_number',
                'purchase_orders.document_number as po_number',
                'sales_orders.date as so_date',
                'purchase_orders.date as po_date',
                'categories.name as category_name',
                'subcategories.sub_category as sub_category_name',
                'po_items.po_item_no',
                'so_items.so_item_no',
                'po_items.qty as po_qty',
                'so_items.qty as so_qty',
                'po_items.id as po_item_id',
                'so_items.id as so_item_id',
                'po_items.unit_price as po_item_unit_price',
                'so_items.unit_price as so_item_unit_price',
                'dispatches.id as dispatch_id',
                'po_items.id as po_item_id',
                'so_items.id as so_item_id',
            )
            ->where('dispatches.dispatch_number', $dispatch_number)
            ->get();


        $dispatch_data_check = Dispatch::leftJoin('so_items', 'dispatches.so_item_id', '=', 'so_items.id')
            ->leftJoin('po_items', 'dispatches.po_item_id', '=', 'po_items.id')
            ->leftJoin('companies as po_company', 'dispatches.po_company_id', '=', 'po_company.id')
            ->leftJoin('companies as so_company', 'dispatches.so_company_id', '=', 'so_company.id')
            ->leftJoin('sales_orders', 'dispatches.so_id', '=', 'sales_orders.id')
            ->leftJoin('purchase_orders', 'dispatches.po_id', '=', 'purchase_orders.id')
            ->leftJoin('categories', 'dispatches.category_id', '=', 'categories.id')
            ->leftJoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
            ->select(
                'dispatches.*',
            )
            ->where('dispatches.dispatch_number', $dispatch_number)
            ->first();


        $dispatch_company = Dispatch::leftJoin('companies as po_company', 'dispatches.po_company_id', '=', 'po_company.id')
            ->leftJoin('companies as so_company', 'dispatches.so_company_id', '=', 'so_company.id')
            ->select(
                'dispatches.*',
                'po_company.company_name as po_company',
                'so_company.company_name as so_company',
                'po_company.id as po_company_id',
                'so_company.id as so_company_id',
            )
            ->where('dispatches.dispatch_number', $dispatch_number)
            ->first();


        $dispatch_po_company = $dispatch_company->po_company;
        $dispatch_so_company = $dispatch_company->so_company;

        $dispatch_po_company_id = $dispatch_company->po_company_id;
        $dispatch_so_company_id = $dispatch_company->so_company_id;

        $dispatch_date = $dispatch_company->date;
        $dispatch_vehicle = $dispatch_company->vehicle_number;

        // Fetch po_items and so_items

        $subcategories = Subcategory::all();


        if ($dispatch_data_check->dispatch_type == 'DispatchSO') {

            $po_item_ids = $dispatch_data->pluck('po_item_id')->toArray();
            $so_item_ids = $dispatch_data->pluck('so_item_id')->toArray();

            $po_items = Dispatch::join('po_items', 'dispatches.po_item_id', '=', 'po_items.id')
                ->join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                ->join('categories', 'po_items.item_category', '=', 'categories.id')
                ->leftJoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
                ->select(
                    'categories.name',
                    'purchase_orders.document_number',
                    'purchase_orders.date',
                    'po_items.id as po_item_id',
                    'po_items.item_category',
                    'po_items.po_item_no',
                    'po_items.qty',
                    'po_items.po_dispatch_rest_qty',
                    'po_items.unit_price',
                    DB::raw('SUM(dispatches.dispatched_quantity) as total_dispatched_qty')
                )
                ->whereIn('po_items.id', $po_item_ids)
                ->where('dispatches.dispatch_number', $dispatch_number)
                ->groupBy(
                    'po_items.id',  // Group by po_item_id
                    'categories.name',  // Group by category name
                    'po_items.item_category',
                    'purchase_orders.document_number',  // Group by document number
                    'purchase_orders.date',
                    'po_items.po_item_no',
                    'po_items.qty',
                    'po_items.po_dispatch_rest_qty',
                    'po_items.unit_price'
                )
                ->get();


            $so_items = Dispatch::join('so_items', 'dispatches.so_item_id', '=', 'so_items.id')
                ->join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->join('categories', 'so_items.item_category', '=', 'categories.id')
                ->leftJoin('subcategories', 'dispatches.subcategory_id', '=', 'subcategories.id')
                ->select(
                    'categories.name',
                    'sales_orders.so_number',
                    'sales_orders.date',
                    'so_items.id as so_item_id',
                    'so_items.item_category',
                    'so_items.so_item_no',
                    'so_items.qty',
                    'so_items.so_dispatch_rest_qty',
                    'so_items.unit_price',
                    'subcategories.sub_category as sub_category_name',
                    'dispatches.dispatch_number',
                    'dispatches.conv_rate',
                    'dispatches.dispatch_other',
                    'dispatches.subcategory_id',
                    DB::raw('SUM(dispatches.dispatched_quantity) as total_dispatched_qty')
                )
                ->whereIn('so_items.id', $so_item_ids)
                ->where('dispatches.dispatch_number', $dispatch_number)
                ->groupBy(
                    'so_items.id',  // Group by so_item_id
                    'so_items.item_category',
                    'sales_orders.date',
                    'dispatches.subcategory_id',  // Group by subcategory_id
                    'categories.name',  // Group by category name
                    'sales_orders.id',  // Group by sales order id
                    'subcategories.sub_category',  // Group by subcategory name
                    'dispatches.dispatch_number',  // Group by dispatch number
                    'sales_orders.so_number',
                    'so_items.so_item_no',
                    'so_items.qty',
                    'so_items.so_dispatch_rest_qty',
                    'so_items.unit_price',
                    'dispatches.conv_rate',
                    'dispatches.dispatch_other',

                )
                ->orderBy('dispatches.id', 'desc')
                ->get();





            // Loop through each dispatch_data row



            // Now, each $po_item will have its corresponding dispatched quantity in `dispatch_po_qty`
            // Similarly, each $so_item will have its dispatched quantity in `dispatch_so_qty`


            // Now each `so_item` in `$so_items` will have the summed `dispatch_so_qty`


            // Calculate the total number of SO items
            $total_no = count($so_items);

            $dispatch_po_company = $dispatch_company->po_company;
            $dispatch_so_company = $dispatch_company->so_company;

            return view('newdispatch_so.edit', [
                'dispatch_data' => $dispatch_data,
                'sub_items' => null, // Replace with actual data if available
                'so_items' => $so_items,
                'po_items' => $po_items,
                'dispatch_po_price_gross' => null, // Replace with actual data if available
                'dispatch_so_price_gross' => null, // Replace with actual data if available
                'dispatch_po_price' => null, // Replace with actual data if available
                'dispatch_so_price' => null, // Replace with actual data if available
                'freight_insurance' => null, // Replace with actual data if available
                'dispatch_po_company' => $dispatch_po_company,
                'dispatch_so_company' => $dispatch_so_company,
                'dispatch_date' => $dispatch_date,
                'dispatch_vehicle' =>  $dispatch_vehicle,
                'subcategories' => $subcategories,
                'dispatch_number' => $dispatch_number,
                'total_no' => $total_no,
                'dispatch_po_company_id' =>  $dispatch_po_company_id,
                'dispatch_so_company_id' =>  $dispatch_so_company_id,
            ]);
        } else {

            $po_items = [];
            $so_items = [];

            $dispatch_data->each(function ($dispatch) use (&$po_items, &$so_items) {
                // Fetch the PoItem and SoItem
                $po_item = PoItem::join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                    ->Join('categories', 'po_items.item_category', '=', 'categories.id')
                    ->select('categories.name', 'po_items.*', 'po_items.id as po_item_id', 'purchase_orders.*')
                    ->where('po_items.id', $dispatch->po_item_id)->first();

                $so_item = SoItem::join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                    ->Join('categories', 'so_items.item_category', '=', 'categories.id')
                    ->select('categories.name', 'so_items.*', 'so_items.id as so_item_id', 'sales_orders.*')
                    ->where('so_items.id', $dispatch->so_item_id)->first();

                // Add PoItem if its po_item_no is not already in the array
                if (!in_array($po_item->po_item_no, array_column($po_items, 'po_item_no'))) {
                    $po_items[] = $po_item;
                }

                // Add SoItem if its so_item_no is not already in the array
                if (!in_array($so_item->so_item_no, array_column($so_items, 'so_item_no'))) {
                    $so_items[] = $so_item;
                }
            });


            return view('newdispatch.edit', [
                'dispatch_data' => $dispatch_data,
                'sub_items' => null, // Replace with actual data if available
                'so_items' => $so_items,
                'po_items' => $po_items,
                'dispatch_po_price_gross' => null, // Replace with actual data if available
                'dispatch_so_price_gross' => null, // Replace with actual data if available
                'dispatch_po_price' => null, // Replace with actual data if available
                'dispatch_so_price' => null, // Replace with actual data if available
                'freight_insurance' => null, // Replace with actual data if available
                'dispatch_po_company' => $dispatch_po_company,
                'dispatch_so_company' => $dispatch_so_company,
                'dispatch_date' => $dispatch_date,
                'dispatch_vehicle' =>  $dispatch_vehicle,
                'subcategories' => $subcategories,
                'dispatch_number' => $dispatch_number,
            ]);
        }
    }


    public function updateDispatch(Request $request)
    {

        $rowIdentifiers = [];
        $soDispatchQuantities = [];
        $poDispatchQuantities = [];

        foreach ($request->quantity as $index => $quantity) {
            $rowKey = $request->po_item_number[$index] . '-' .
                $request->sub_cat_id[$index] . '-' .
                $request->so_item_no[$index];

            // Check for duplicate rows
            if (in_array($rowKey, $rowIdentifiers)) {
                return response()->json([
                    'error' => 'Duplicate rows detected for PO Item, Subcategory, or SO Item.'
                ], 400);  // 400 is for Bad Request
            }

            $rowIdentifiers[] = $rowKey;

            // Fetch SO and PO items
            $so_item = SoItem::where('so_item_no', $request->so_item_no[$index])->first();
            $po_item = PoItem::where('po_item_no', $request->po_item_number[$index])->first();

            if (!$so_item || !$po_item) {
                return response()->json([
                    'error' => 'Please select at least one PO Item or SO Item.'
                ], 400);
            }

            // Initialize cumulative dispatched quantities
            $soDispatchQuantities[$request->so_item_no[$index]] = $soDispatchQuantities[$request->so_item_no[$index]] ?? 0;
            $poDispatchQuantities[$request->po_item_number[$index]] = $poDispatchQuantities[$request->po_item_number[$index]] ?? 0;

            // Update cumulative dispatched quantities
            $soDispatchQuantities[$request->so_item_no[$index]] += $quantity;
            $poDispatchQuantities[$request->po_item_number[$index]] += $quantity;

            $dispatchedSOQty = round(floatval($soDispatchQuantities[$request->so_item_no[$index]]), 3);
            $dispatchedPOQty = round(floatval($poDispatchQuantities[$request->po_item_number[$index]]), 3);

            // Validation: Compare (so_dispatch_rest_qty + $quantity) with dispatched quantities
            $soAllowedQty = round(floatval($so_item->so_dispatch_rest_qty + $quantity), 3);
            $poAllowedQty = round(floatval($po_item->po_dispatch_rest_qty + $quantity), 3);

            if ($dispatchedSOQty > $soAllowedQty) {
                return response()->json([
                    'error' => 'Dispatched quantity for SO Item ' . $so_item->so_item_no . ' exceeds the allowed quantity (' . number_format($soAllowedQty, 3) . ').'
                ], 400);
            }

            if ($dispatchedPOQty > $poAllowedQty) {
                return response()->json([
                    'error' => 'Dispatched quantity for PO Item ' . $po_item->po_item_no . ' exceeds the allowed quantity (' . number_format($poAllowedQty, 3) . ').'
                ], 400);
            }
        }

        $old_dispatch = Dispatch::where('dispatch_number', $request->dispatch_number)->get();
        // Restore previous quantities
        foreach ($old_dispatch as $old_data) {
            SoItem::where('id', $old_data->so_item_id)->increment('so_dispatch_rest_qty', $old_data->dispatched_quantity);
            PoItem::where('id', $old_data->po_item_id)->increment('po_dispatch_rest_qty', $old_data->dispatched_quantity);
            Dispatch::where('id', $old_data->id)->delete();
        }

        // Save or update dispatch data
        foreach ($request->quantity as $index => $quantity) {
            $so_item = SoItem::where('so_item_no', $request->so_item_no[$index])
                ->join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->select('sales_orders.*', 'so_items.*', 'so_items.id as so_item_id')
                ->first();
            $po_item = PoItem::where('po_item_no', $request->po_item_number[$index])
                ->join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                ->select('purchase_orders.*', 'po_items.*', 'po_items.id as po_item_id')
                ->first();
            $dispatch = Dispatch::updateOrCreate(
                ['dispatch_number' => $request->dispatch_number, 'so_item_id' => $request->so_item_no[$index]], // Condition to identify specific row
                [
                    'date' => $request->date,
                    'po_company_id' => $po_item->supplier_id,
                    'so_company_id' => $so_item->company_id,
                    'po_id' => $po_item->po_id,
                    'so_id' => $so_item->so_id,
                    'po_item_id' => $po_item->po_item_id,
                    'so_item_id' => $so_item->so_item_id,
                    'category_id' => $request->cat_id[$index],
                    'subcategory_id' => $request->sub_cat_id[$index],
                    'dispatched_quantity' => $request->quantity[$index],
                    'conv_rate' => $request->conv_rate[$index],
                    'dispatch_unit_price' => $request->dispatch_unit_price_actual[$index],
                    'dispatch_other' => $request->dispatch_fregiht_insuance[$index],
                    'dispatch_so_other' => $request->dispatch_fregiht_insuance[$index],
                    'dispatch_total' => $request->dispatch_total[$index],
                    'vehicle_number' => $request->vehicle_number,
                    'dispatch_so_unit_price' => $so_item->unit_price,
                    'dispatch_so_total' => $request->dispatch_so_total[$index],
                    'receiver_person' => $request->receiver_person,
                    'remarks' => $request->remarks,
                ]
            );

            // Update the remaining quantities
            $actual_so_dispatch_qty = number_format($so_item->so_dispatch_rest_qty - $dispatch->dispatched_quantity, 3);
            $actual_po_dispatch_qty = number_format($po_item->po_dispatch_rest_qty - $dispatch->dispatched_quantity, 3);

            $so_item->update(['so_dispatch_rest_qty' => $actual_so_dispatch_qty]);
            $po_item->update(['po_dispatch_rest_qty' => $actual_po_dispatch_qty]);

            // Update item statuses
            $so_item->update(['so_dispatch_item_status' => $actual_so_dispatch_qty == 0 ? 'Close' : 'Partial Pending']);
            $po_item->update(['po_dispatch_item_status' => $actual_po_dispatch_qty == 0 ? 'Close' : 'Partial Pending']);
        }

        return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);

        // return redirect()->route('dispatch.index')->with('update', 'Dispatch updated successfully.');
    }


    public function destroyDispatch($id)
    {

        $old_dispatch = Dispatch::where('id', $id)->first();

        $so_item = SoItem::where('id', $old_dispatch->so_item_id)->first();
        $po_item = PoItem::where('id', $old_dispatch->po_item_id)->first();

        $old_so_rest_qty = ($old_dispatch->dispatched_quantity + $so_item->so_dispatch_rest_qty);
        $old_po_rest_qty = ($old_dispatch->dispatched_quantity + $po_item->po_dispatch_rest_qty);

        $so_item->update(['so_dispatch_rest_qty' => $old_so_rest_qty]);
        $po_item->update(['po_dispatch_rest_qty' => $old_po_rest_qty]);

        if ($old_so_rest_qty < $so_item->qty) {
            $so_item->update(['so_dispatch_item_status' => 'Partial Pending']);
        } else {
            $so_item->update(['so_dispatch_item_status' => 'Open']);
        }
        if ($old_po_rest_qty < $po_item->qty) {
            $po_item->update(['po_dispatch_item_status' => 'Partial Pending']);
        } else {
            $po_item->update(['po_dispatch_item_status' => 'Open']);
        }

        Dispatch::where('id', $id)->delete();
        return redirect()->route('dispatch.index')->with('delete', 'Dispatch deleted successfully.');
    }

    public function get_conv_price(Request $request)
    {
        $cov_rates = ConvRate::where('subcategory_id', $request->subcategory_item_id)->latest()->first();
        return response($cov_rates);
    }

    public function get_dispatch_payable_total(Request $request)
    {

        $dispatch_data = Dispatch::where('id', $request->dispatch_id)->first();
        // dd($dispatch_data);
        return response($dispatch_data);
        // return response()->json([
        //     'rows_data' => $dispatch_data
        // ]);

    }
    public function get_dispatch_so_unit_price(Request $request)
    {
        // Fetch the dispatch data based on the given dispatch ID
        $dispatch_data = Dispatch::where('id', $request->dispatch_id)->first();

        // Get the total quantity from the 'dispatch_total' field
        $totalQty = $dispatch_data->dispatch_so_total ?? 0; // Use null coalescing operator to handle null cases

        // Return the data as a structured JSON response
        return response()->json([
            'total_qty' => $totalQty,
            'dispatch_so_unit_price' => $dispatch_data->dispatch_so_unit_price ?? 0,
            'conv_rate' => $dispatch_data->conv_rate ?? 0,
            'dispatch_freight' => $dispatch_data->dispatch_freight ?? 0,
            'dispatch_other' => $dispatch_data->dispatch_other ?? 0,
            'dispatched_quantity' => $dispatch_data->dispatched_quantity ?? 0,
        ]);
    }

    public function get_so_unit_price(Request $request)
    {
        $soItemNo = $request->input('so_item_no');
        $item = SOItem::where('so_item_no', $soItemNo)->first();

        if ($item) {
            return response()->json([
                'success' => true,
                'unit_price' => $item->unit_price,
                'qty' => $item->so_dispatch_rest_qty,
            ]);
        } else {
            return response()->json([
                'success' => false,
                'message' => 'SO item not found',
            ]);
        }
    }



    public function main_dispatch_redirect(Request $request)
    {

        $soItemNos = $request->input('so_item_no');
        $subCatIds = $request->input('sub_cat_id');

        // Combine so_item_no and sub_cat_id for validation
        $validationData = [];
        foreach ($soItemNos as $index => $soItemNo) {
            $subCatId = $subCatIds[$index] ?? null;
            $validationData[$soItemNo][] = $subCatId;
        }

        // Check for duplicate sub_cat_id within each so_item_no
        $errors = [];
        foreach ($validationData as $soItemNo => $subCatList) {
            $duplicates = array_filter(array_count_values($subCatList), fn($count) => $count > 1);
            if (!empty($duplicates)) {
                $errors[$soItemNo] = array_keys($duplicates);
            }
        }

        // If errors exist, return with error message
        if (!empty($errors)) {
            return response()->json(['message' => 'Conv item is duplicate for one so item.'], 400);
        }

        // $total_dispatch_so = 0;
        //  foreach($request->so_item_no as $so_item_no){
        //     $so_item = SoItem::where('so_item_no', $so_item_no)->sum('so_dispatch_rest_qty');
        //  }

        $po_items = PoItem::join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
            ->join('categories', 'categories.id', '=', 'po_items.item_category')
            ->select('purchase_orders.*', 'categories.*', 'po_items.*', 'po_items.price as po_price', 'po_items.po_dispatch_rest_qty', 'po_items.po_id', 'po_items.id as po_item_id')
            ->whereIn('po_item_no', $request->po_item_no)->get();



        $dispatch_so_qty = $request->dispatch_so_qty;
        $dispatch_po_qty = $request->dispatch_po_qty;

        $sub_cat_id = $request->sub_cat_id;



        $so_items = [];
        foreach ($request->so_item_no as $key => $data) {
            $so_item = SoItem::join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->join('categories', 'categories.id', '=', 'so_items.item_category')
                ->select(
                    'sales_orders.*',
                    'categories.*',
                    'so_items.*',
                    'so_items.price as so_price',
                    'so_items.so_id as so_id',
                    'so_items.id as so_item_id'
                )
                ->where('so_item_no', $data)
                ->first();

            if ($so_item) {
                // Fetch the corresponding subcategory
                $sub_category = SubCategory::find($request->sub_cat_id[$key] ?? null);
                if ($sub_category) {
                    // Fetch the latest conversion rate
                    $conv_rate = ConvRate::where('subcategory_id', $sub_category->id)->latest()->first();

                    // Attach additional properties to the SO item
                    $so_item->sub_cat_id = $sub_category->id;
                    $so_item->sub_cat_name = $sub_category->sub_category;
                    $so_item->conv_rate = $conv_rate->item_price ?? 0;
                } else {
                    // Default values if no subcategory found
                    $so_item->sub_cat_id = null;
                    $so_item->sub_cat_name = null;
                    $so_item->conv_rate = 0;
                }

                // Assign dispatch quantity to each SO item
                $so_item->dispatch_so_qty = $request->dispatch_so_qty[$key] ?? 0;

                // Add the item to the result array
                $so_items[] = $so_item;
            }
        }

        // Calculate the total number of SO items
        $total_no = count($so_items);

        foreach ($po_items as $key => $po_item) {
            $po_item->dispatch_po_qty = $dispatch_po_qty[$key] ?? 0; // Assign dispatch quantity to each SO item
        }

        // dd( $so_items);

        $dispatch_po_company = Company::where('id', $request->po_company_id)->first();
        $dispatch_so_company = Company::where('id', $request->so_company_id)->first();

        session([
            'po_items' => $po_items,
            'so_items' => $so_items,
            'total_no' => $total_no,
            'dispatch_po_company' => $dispatch_po_company,
            'dispatch_so_company' => $dispatch_so_company
        ]);

        return response()->json(['redirect' => route('dispatch_so.main_create')]);
    }


    public function storeDispatchSO(Request $request)
    {

        //    dd($request);
        // ......................................................................................so_check.......................................................................
        $total_dispatch_so = 0;

        foreach ($request->quantity as $quantity) {
            $total_dispatch_so += $quantity;
        }

        $so_items_qty_total = SoItem::whereIn('id', $request->so_item_id)->sum('so_dispatch_rest_qty');

        if ($so_items_qty_total < $total_dispatch_so) {
            return response()->json([
                'message' => 'Dispatched quantity for SO Item Exceed (' . number_format($so_items_qty_total, 3) . ').'
            ], 400);
        }

        // Validate PO dispatch quantity


        //  ............................................................................................................................................................................


        // ......................................................................................po_check.......................................................................

        $total_dispatch_po = 0;

        foreach ($request->qty as $quantity) {
            $total_dispatch_po += reset($quantity); // Get the first value from the associative array
        }


        $po_items_qty_total = PoItem::whereIn('po_item_no', $request->po_item_no)->sum('po_dispatch_rest_qty');

        if ($po_items_qty_total < $total_dispatch_po) {
            return response()->json([
                'message' => 'Dispatched quantity for PO Item Exceed(' . number_format($po_items_qty_total, 3) . ').'
            ], 400);
        }

        //  ............................................................................................................................................................................


        // ......................................................................................so_po_qty_check.......................................................................

        if ($total_dispatch_po !=  $total_dispatch_so) {
            return response()->json([
                'message' => 'Total Dispatched quantity for PO Item and SO Item Not equal.'
            ], 400);
        }
        //  ............................................................................................................................................................................



        // ......................................................................................Taxonomy.......................................................................

        $year = date('Y');
        $month = date('m');

        // Financial year calculation
        if ($month >= 4) {
            $financial_year_start = $year;
            $financial_year_end = $year + 1;
        } else {
            $financial_year_start = $year - 1;
            $financial_year_end = $year;
        }

        // Financial year format
        $financial_year = $financial_year_start;

        // Fetch the latest dispatch number for the current financial year
        $last_dispatch = Dispatch::whereBetween('created_at', [
            "$financial_year_start-04-01 00:00:00",
            "$financial_year_end-03-31 23:59:59",
        ])->orderBy('dispatch_number', 'desc')->first();

        if ($last_dispatch) {
            $max_serial_number = $last_dispatch->dispatch_number;
            $last_serial_number = substr($max_serial_number, -4); // Get the last 4 digits
            $next_serial_number = str_pad((int)$last_serial_number + 1, 4, '0', STR_PAD_LEFT);
        } else {
            $next_serial_number = '0001'; // Default serial number when no records exist
        }

        // Generate Dispatch number with financial year
        $doc_number = 'DIS' . $financial_year . $next_serial_number;




        //  ............................................................................................................................................................................


        //   ...........................................................................Save Data..................................................................


        foreach ($request->qty as $index => $qty) {

            $so_item_id = array_key_first($qty);

            $dispatch_qty = $qty[$so_item_id];
            if ($dispatch_qty == 0) {
                continue;
            }

            $so_item_index = array_search($so_item_id, $request->so_item_id);

            $so_item = SoItem::where('so_items.id', $so_item_id)
                ->join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->select('sales_orders.*', 'so_items.*', 'so_items.id as so_item_id')
                ->first();


            $po_item = PoItem::where('po_item_no', $request->po_item_no[$index])
                ->join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                ->select('purchase_orders.*', 'po_items.*', 'po_items.id as po_item_id')
                ->first();

            // dd( $po_item);


            $dispatch = new Dispatch();
            $dispatch->date = $request->date;
            $dispatch->dispatch_type = 'DispatchSO';
            $dispatch->dispatch_number = $doc_number;
            $dispatch->po_company_id =  $request->po_company_id;
            $dispatch->so_company_id =  $request->so_company_id;
            $dispatch->po_id = $po_item->po_id;
            $dispatch->so_id = $so_item->so_id;
            $dispatch->po_item_id = $po_item->po_item_id;
            $dispatch->so_item_id = $so_item->so_item_id;

            $dispatch->category_id = $so_item->item_category;

            $dispatch->subcategory_id = $request->sub_cat_id_po[$index] ?? 'Select Item';

            $dispatch->dispatched_quantity = $dispatch_qty;

            $dispatch->conv_rate = $request->conv_rate_po[$index];

            $dispatch->dispatch_unit_price = $po_item->unit_price;

            $dispatch->dispatch_other = $request->dispatch_freight_insuance_po[$index];

            $dispatch->dispatch_so_other = $request->dispatch_freight_insuance_po[$index];

            $dispatch_total_po =  (($request->conv_rate_po[$index] + $request->dispatch_freight_insuance_po[$index] + $po_item->unit_price) * $dispatch_qty);

            $dispatch->dispatch_total = $dispatch_total_po;

            $dispatch->vehicle_number = $request->vehicle_number;

            $dispatch->dispatch_so_unit_price = $so_item->unit_price;

            $dispatch_total_so =  (($request->conv_rate_po[$index] + $request->dispatch_freight_insuance_po[$index] + $so_item->unit_price) * $dispatch_qty);

            $dispatch->dispatch_so_total = $dispatch_total_so;

            $dispatch->receiver_person = $request->receiver_person;

            $dispatch->remarks = $request->remarks;
            $dispatch->save();

            $actual_so_dispatch_qty = number_format($so_item->so_dispatch_rest_qty - $dispatch_qty, 3);
            $actual_po_dispatch_qty = number_format($po_item->po_dispatch_rest_qty - $dispatch_qty, 3);


            $so_item->update(['so_dispatch_rest_qty' => $actual_so_dispatch_qty]);
            $po_item->update(['po_dispatch_rest_qty' => $actual_po_dispatch_qty]);

            if ($actual_so_dispatch_qty == 0) {
                $so_item->update(['so_dispatch_item_status' => 'Close']);
            } else {
                $so_item->update(['so_dispatch_item_status' => 'Partial Pending']);
            }
            if ($actual_po_dispatch_qty == 0) {
                $po_item->update(['po_dispatch_item_status' => 'Close']);
            } else {
                $po_item->update(['po_dispatch_item_status' => 'Partial Pending']);
            }
        }

        // dd(1);

        // return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);


        return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);

        // return redirect()->route('dispatch.index')->with('success', 'Dispatch details saved successfully.');
    }


    public function updateDispatchSO(Request $request)
    {
        $dispatch_data = Dispatch::where('dispatch_number', $request->dispatch_number)->get();

        // ......................................................................................so_check.......................................................................
        $total_dispatch_so = 0;

        foreach ($request->quantity as $quantity) {
            $total_dispatch_so += $quantity;
        }

        $total_rest_dispatch_so = 0;
        foreach ($request->dispatch_so_qty as $dispatch_so_qty) {
            $total_rest_dispatch_so += $dispatch_so_qty;
        }


        $so_items_qty_total = SoItem::whereIn('id', $request->so_item_id)->sum('so_dispatch_rest_qty');

        if (($total_rest_dispatch_so) < $total_dispatch_so) {
            return response()->json([
                'message' => 'Dispatched quantity for SO Item Exceed (' . number_format($so_items_qty_total, 3) . ').'
            ], 400);
        }

        // Validate PO dispatch quantity


        //  ............................................................................................................................................................................


        // ......................................................................................po_check.......................................................................

        $total_dispatch_po = 0;

        foreach ($request->qty as $quantity) {
            $total_dispatch_po += reset($quantity); // Get the first value from the associative array
        }

        $po_items_qty_already_dispatch = Dispatch::whereIn('po_item_id', $request->po_item_select)->where('dispatch_number', $request->dispatch_number)->sum('dispatched_quantity');

        $po_items_qty_total = PoItem::whereIn('po_item_no', $request->po_item_no)->sum('po_dispatch_rest_qty');

        if (($po_items_qty_total + $po_items_qty_already_dispatch) < $total_dispatch_po) {
            return response()->json([
                'message' => 'Dispatched quantity for PO Item Exceed(' . number_format($po_items_qty_total, 3) . ').'
            ], 400);
        }

        //  ............................................................................................................................................................................


        // ......................................................................................so_po_qty_check.......................................................................

        if ($total_dispatch_po !=  $total_dispatch_so) {
            return response()->json([
                'message' => 'Total Dispatched quantity for PO Item and SO Item Not equal.'
            ], 400);
        }
        //  ............................................................................................................................................................................


        //   ...........................................................................Save Data..................................................................

        // .........................................................................................deleteoldDispatch.......................................................
        foreach ($dispatch_data as $old_data) {
            SoItem::where('id', $old_data->so_item_id)->increment('so_dispatch_rest_qty', $old_data->dispatched_quantity);
            PoItem::where('id', $old_data->po_item_id)->increment('po_dispatch_rest_qty', $old_data->dispatched_quantity);
            Dispatch::where('id', $old_data->id)->delete();
        }
        //  ............................................................................................................................................................................

        foreach ($request->qty as $index => $qty) {

            $so_item_id = array_key_first($qty);

            $dispatch_qty = $qty[$so_item_id];
            if ($dispatch_qty == 0) {
                continue;
            }

            $so_item_index = array_search($so_item_id, $request->so_item_id);

            $so_item = SoItem::where('so_items.id', $so_item_id)
                ->join('sales_orders', 'so_items.so_id', '=', 'sales_orders.id')
                ->select('sales_orders.*', 'so_items.*', 'so_items.id as so_item_id')
                ->first();


            $po_item = PoItem::where('po_item_no', $request->po_item_no[$index])
                ->join('purchase_orders', 'po_items.po_id', '=', 'purchase_orders.id')
                ->select('purchase_orders.*', 'po_items.*', 'po_items.id as po_item_id')
                ->first();

            // dd( $po_item);


            $dispatch = new Dispatch();
            $dispatch->date = $request->date;
            $dispatch->dispatch_type = 'DispatchSO';
            $dispatch->dispatch_number = $request->dispatch_number;
            $dispatch->po_company_id =  $request->po_company_id;
            $dispatch->so_company_id =  $request->so_company_id;
            $dispatch->po_id = $po_item->po_id;
            $dispatch->so_id = $so_item->so_id;
            $dispatch->po_item_id = $po_item->po_item_id;
            $dispatch->so_item_id = $so_item->so_item_id;

            $dispatch->category_id = $so_item->item_category;

            $dispatch->subcategory_id = $request->sub_cat_id_po[$index] ?? 'Select Item';

            $dispatch->dispatched_quantity = $dispatch_qty;

            $dispatch->conv_rate = $request->conv_rate_po[$index];

            $dispatch->dispatch_unit_price = $po_item->unit_price;

            $dispatch->dispatch_other = $request->dispatch_freight_insuance_po[$index];

            $dispatch->dispatch_so_other = $request->dispatch_freight_insuance_po[$index];

            $dispatch_total_po =  (($request->conv_rate_po[$index] + $request->dispatch_freight_insuance_po[$index] + $po_item->unit_price) * $dispatch_qty);

            $dispatch->dispatch_total = $dispatch_total_po;

            $dispatch->vehicle_number = $request->vehicle_number;

            $dispatch->dispatch_so_unit_price = $so_item->unit_price;

            $dispatch_total_so =  (($request->conv_rate_po[$index] + $request->dispatch_freight_insuance_po[$index] + $so_item->unit_price) * $dispatch_qty);

            $dispatch->dispatch_so_total = $dispatch_total_so;

            $dispatch->receiver_person = $request->receiver_person;

            $dispatch->remarks = $request->remarks;
            $dispatch->save();

            $actual_so_dispatch_qty = number_format($so_item->so_dispatch_rest_qty - $dispatch_qty, 3);
            $actual_po_dispatch_qty = number_format($po_item->po_dispatch_rest_qty - $dispatch_qty, 3);


            $so_item->update(['so_dispatch_rest_qty' => $actual_so_dispatch_qty]);
            $po_item->update(['po_dispatch_rest_qty' => $actual_po_dispatch_qty]);

            if ($actual_so_dispatch_qty == 0) {
                $so_item->update(['so_dispatch_item_status' => 'Close']);
            } else {
                $so_item->update(['so_dispatch_item_status' => 'Partial Pending']);
            }
            if ($actual_po_dispatch_qty == 0) {
                $po_item->update(['po_dispatch_item_status' => 'Close']);
            } else {
                $po_item->update(['po_dispatch_item_status' => 'Partial Pending']);
            }
        }

        // dd(1);

        // return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);


        return response()->json(['success' => true, 'redirect' => route('dispatch.index')]);
    }

    public function DispatchSOmain()
    {
        // Retrieve the data from session
        $po_items = session('po_items', []);
        $so_items = session('so_items', []);
        $total_no = session('total_no', 0);
        $dispatch_po_company = session('dispatch_po_company');
        $dispatch_so_company = session('dispatch_so_company');

        return view('newdispatch_so.main_create', compact('po_items', 'so_items', 'total_no', 'dispatch_po_company', 'dispatch_so_company'));
    }
}
