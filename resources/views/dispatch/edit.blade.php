...
@section('content')
    ...
    <script>
        ...
        var filteredSubItems = subcategories.filter(sub => sub.category_id == {{ $data->category_id }});
            ...
            <input type="hidden" name="cat_id[]" class="form-control" value="{{ $data->category_id }}" required>
        `;
    ...
    <td>
        ...
        <input type="hidden" name="dispatch_so_unit_price_actual[]" id="so_unit_price_actual" value="{{ $data->dispatch_so_unit_price }}" class="form-control" readonly required />
         ...
         <input type="hidden"  name="po_item_number[]" value="{{$data->po_item_no}}">
    </td>
`;

      cell7.innerHTML = `
            <td><input type="number" name="po_rest_qty_show"  oninput="calculateTotal(this)" step="0.001" min="0.001" class="form-control" value="{{ $data->po_dispatch_rest_qty +  $data->dispatched_quantity}}" readonly /></td>
        `;
...
cell11.innerHTML = `
            <td><input type="number" name="so_rest_qty_show[]"   step="0.001" min="0.001" class="form-control" value="{{ $data->so_dispatch_rest_qty +  $data->dispatched_quantity}}" readonly /></td>
        `;
