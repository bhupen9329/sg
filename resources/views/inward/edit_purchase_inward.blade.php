...
`;
            ...
            <select name="item_sub_category[]" id="item_sub_category${lastItemId}" onchange="get_subcategory_list(this) ; check_same_data('${lastItemId}') " style="height: 34px; width: 220px;" class="form-control subcategory-select sub_category-item-select-${lastItemId}" required>
                ...
                @endforeach
                                                        ...
                                                        cell3.innerHTML =
                                                            `
            <input type="number" min="1" name="length[]" id="length_${lastItemId}" value="{{ $inw_item->length }}" class="form-control length-input" style="height: 34px; width: 220px;" placeholder="Length" oninput="change_status('${lastItemId}'); check_same_data('${lastItemId}'); get_current_quantity(this);" required >`;

                                                        cell4.innerHTML =
                                                            `
            <input type="number" min="1" id="pcs_${lastItemId}" value="{{ $inw_item->piece }}" oninput="change_status('${lastItemId}')"  name="piece[]" class="form-control price-input" style="height: 34px; width: 220px;" placeholder="PCs" required>`;
                                                        cell5.innerHTML =
                                                            `
            ...
            <input type="hidden" id="weight_hidden_${lastItemId}" value="{{ $inw_item->subcategory_weight }}"  class="form-control weight-input2" style="height: 34px; width: 101px" placeholder="weight" onchange="calculateTotal()" required readonly>`;
        ...
        `;
            ...
            <input type="number" min="1" name="length[]" id="length_${lastItemId}" class="form-control length-input" style="height: 34px" placeholder="Length" oninput="change_status('${lastItemId}'); check_same_data('${lastItemId}'); get_current_quantity(this); get_weight_in_edit(this)"required >`;
                                                    ...
                                                    cell4.innerHTML =
                                                        `
            <input type="number" min="1" id="pcs_${lastItemId}" name="piece[]" class="form-control price-input" oninput="change_status('${lastItemId}'); get_weight_in_edit(this)" style="height: 34px" placeholder="PCs" required >`;
                                                    cell5.innerHTML =
                                                        `
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-6 mb-2">
                                                        <label for="inputPassword3" class="  col-form-label"><strong>
                                                                Auto Calculated Weight</strong> </label>
                                                    </div>
                                                    <div class="col-lg-6">
                                                    ...
                                                    <p id="error_box" style="display:none; color:red;"> Alert! Weight
                                                        difference percent exceeded the limit.</p>
                                                    <style>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            calculateShortageOnInput();
            // calculateforshortagemessage();
        });

        // document.getElementById("godown_weight").addEventListener("input", calculateforshortagemessage);
        document.getElementById("godown_weight").addEventListener("input", calculateShortageOnInput);
        document.getElementById("plant_weight").addEventListener("input", calculateShortageOnInput);

        function calculateShortageOnInput() {
            var shortageInput = document.getElementById('shortage');
            var company_setting_Input = parseFloat(document.getElementById("company_setting_shortage_value").value);
            // console.log(company_setting_Input);
            var overallTotalWeightInput = parseFloat(document.getElementById("overall_total_weight").value);
            // console.log(overallTotalWeightInput);
            var godown_weightInput = parseFloat(document.getElementById("godown_weight").value);
            // console.log(godown_weightInput);
            var plant_weightInput = parseFloat(document.getElementById("plant_weight").value);
            var shotage_value = godown_weightInput - plant_weightInput || 0;
            // console.log(shotage_value);
            shortageInput.value = shotage_value;

            var check_percenteg = (overallTotalWeightInput - godown_weightInput) * 100 / overallTotalWeightInput;
            checkpercenteg = parseFloat(check_percenteg.toFixed(2));


            // console.log(company_percent);
            if (check_percenteg > company_setting_Input) {
                $('#error_box').show();
                var error_message_input = document.getElementById('error_message');
                // console.log(error_message_input);
                error_message_input.value = true;
            } else {
                $('#error_box').hide();
                var error_message_input = document.getElementById('error_message');
                // console.log(error_message_input);
                error_message_input.value = false;
            }
